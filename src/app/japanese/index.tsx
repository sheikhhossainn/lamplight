...
const GENRES = [
  { id: null, label: 'すべて' },
  { id: '小説', label: '小説' },
  { id: '詩歌・俳句', label: '詩歌・俳句' },
  { id: '評論・エッセイ', label: '評論・随筆' },
  { id: '戯曲', label: '戯曲' },
  { id: '外国文学', label: '外国文学' },
  { id: '哲学・宗教', label: '思想・哲学' },
];

export default function JapaneseLibraryScreen() {
  const { colors, typography, spacing, radius, layout } = useTheme();
  const insets = useSafeAreaInsets();

  const [books, setBooks] = useState<AozoraCatalogItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [loadingMore, setLoadingMore] = useState(false);
  const [page, setPage] = useState(1);
  const [total, setTotal] = useState(17350);
  ...
  const fetchBooks = useCallback(
    async (targetPage = 1, append = false, targetQuery = query, targetGenre = activeGenre) => {
      ...
      try {
        ...
        let fetchedItems: AozoraCatalogItem[] = [];

        if (supabaseUrl && supabaseKey) {
          let url = `${supabaseUrl}/rest/v1/books?source_language=eq.ja&select=id,title,author,cover_url,synopsis,categories,total_chapters&order=id.asc&limit=${limit}&offset=${offset}`;
          ...
          if (res.ok) {
            ...
            if (Array.isArray(rows)) {
              fetchedItems = rows.map((r: any) => ({
                ...
                synopsis: r.synopsis,
                category: Array.isArray(r.categories) && r.categories[0] ? r.categories[0] : '日本文学',
        ...
        if (fetchedItems.length === 0 && targetPage === 1 && !targetQuery) {
          fetchedItems = AOZORA_JAPANESE_BOOKS.map((b) => ({
            id: b.id,
            title: b.title,
            author: b.author,
            coverUrl: b.coverUrl,
        ...
        console.warn('[JapaneseLibraryScreen] fetchBooks failed:', err);
      } finally {
        setLoading(false);
  ...
  const renderBookItem = ({ item, index }: { item: AozoraCatalogItem; index: number }) => {
        ...
        <View style={styles.navRow}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={[typography.metadataCaption, { color: colors.fawn, fontSize: 12 }]}>
              パブリックドメイン日本文学 · {total.toLocaleString()} 冊
            </Text>
          </View>
        </View>
