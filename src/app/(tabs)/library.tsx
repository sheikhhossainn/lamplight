...
export default function LibraryScreen() {
  ...
  // The "All books" shelf, narrowed to the active category filter.
  const shelfBooks = useMemo(() => {
    const list = activeCategory
      ? books.filter((b) => bookCategoryMap.get(b.id)?.includes(activeCategory))
      : books;

    // Default sort order: Books closest to 98% appear first in the background
  ...
  }, [activeCategory, books, bookCategoryMap, coverageMap]);
