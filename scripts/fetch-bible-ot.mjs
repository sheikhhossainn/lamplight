// One-off fetch of the Old Testament (World English Bible — public domain) from
// bible-api.com (free, no-key REST API) into static JSON assets bundled with
// the app. Same approach as fetch-quran.mjs: the text is fixed/canonical, so
// it ships in the binary rather than being fetched/cached on-device.
//
// Also pulls Jamieson-Fausset-Brown (JFB) verse commentary from the HelloAO
// Bible API (bible.helloao.org, free, no-key, no rate limit) — the Bible
// equivalent of the Quran fetch script's Al-Jalalayn tafsir pull. JFB was
// chosen over this API's "matthew-henry" commentary (actually the full
// unabridged Matthew Henry text, thousands of characters per entry) for being
// genuinely concise, same reasoning as Al-Jalalayn over Ibn Kathir. JFB groups
// commentary by verse range rather than one entry per verse; every verse in a
// range gets that range's shared commentary text.
//
// RUN LOCALLY (`npm run fetch:bible-ot`). bible-api.com rate-limits to 15
// requests/30s, and the Old Testament is ~929 chapters — this takes ~30
...
const OUT_DIR = path.join(process.cwd(), 'assets', 'bible');

// Protestant canon Old Testament, in order — hardcoded (not sliced from the
// API's full 66-book list) so a future API change can't silently pull in New
// Testament books.
const OT_BOOK_IDS = [
...
const books = existingBooks.filter((b) => OT_BOOK_IDS.includes(b.id));
...
for (const bookId of OT_BOOK_IDS) {
  ...
  // commentary — older pre-commentary asset files have full chapter counts
  // too, so checking chapter count alone would silently skip re-adding
  // commentary to books fetched before this script pulled JFB.
...
books.sort((a, b) => OT_BOOK_IDS.indexOf(a.id) - OT_BOOK_IDS.indexOf(b.id));
await persist();
console.log(`Wrote ${books.length} books to assets/bible/books.json`);
console.log('Wrote assets/bible/verses.json');
console.log('Done.');
