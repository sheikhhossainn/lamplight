-- Migration: Korean Literature (Gongu Madang / Classics) for PostgreSQL (Supabase)
-- Enables remote hosting and retrieval of public domain Korean literature.

-- 1. Create table for Korean chapters in Supabase
CREATE TABLE IF NOT EXISTS public.korean_chapters (
  ...
  CONSTRAINT uq_korean_chapters_book_idx UNIQUE (book_id, chapter_index)
);

CREATE INDEX IF NOT EXISTS korean_chapters_book_id_idx ON public.korean_chapters (book_id);

-- Enable Row Level Security (read-only for all clients)
ALTER TABLE public.korean_chapters ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Allow public read access to korean_chapters" ON public.korean_chapters;
CREATE POLICY "Allow public read access to korean_chapters"
  ON public.korean_chapters FOR SELECT
  USING (true);

-- 2. Seed Hero Korean Books into public.books
...
(
  ...
  null,
  'gongu-korea',
  'internal://korean/ko-nalgae',
  'https://covers.openlibrary.org/b/id/98329-M.jpg',
  ARRAY['모더니즘 심리소설', '근현대소설'],
  'gongu_korea',
  true,
  true
),
(
  ...
  null,
  'gongu-korea',
  'internal://korean/ko-jindallae',
  'https://covers.openlibrary.org/b/id/2337698-M.jpg',
  ARRAY['서정시선집', '한국시'],
  'gongu_korea',
  true,
  false
...
(
  ...
  null,
  'gongu-korea',
  'internal://korean/ko-yun-dong-ju',
  'https://covers.openlibrary.org/b/id/9186495-M.jpg',
  ARRAY['민족 서정시', '한국시'],
  'gongu_korea',
  true,
  false
...
(
  ...
  null,
  'gongu-korea',
  'internal://korean/ko-unsu-joeun-nal',
  'https://covers.openlibrary.org/b/id/13282906-M.jpg',
  ARRAY['사실주의 단편', '근현대소설'],
  'gongu_korea',
  true,
  false
...
(
  ...
  null,
  'gongu-korea',
  'internal://korean/ko-bom-bom',
  'https://covers.openlibrary.org/b/id/3337711-M.jpg',
  ARRAY['해학소설', '근현대소설'],
  'gongu_korea',
  true,
  false
...
-- 3. Seed Korean Chapter Texts into public.korean_chapters
INSERT INTO public.korean_chapters (book_id, chapter_index, title, slug, content) VALUES
('ko-nalgae', 0, '제1부 — 박제가 되어버린 천재', 'nalgae-ch-1', '“박제가 되어버린 천재”를 아시오? 나는 유쾌하오. 이런 때 연애까지가 유쾌하오.
...
('ko-nalgae', 1, '제2부 — 아내와 은화', 'nalgae-ch-2', '아내에게 손님이 찾아오는 날이면, 나는 장롱 속에 웅크리고 숨거나 이불을 뒤집어쓰고 잠든 척해야 했소. 손님이 돌아가고 나면 아내는 내게 은화 한 닢을 쥐여주었소.
...
('ko-nalgae', 2, '제3부 — 날개야 다시 돋아라', 'nalgae-ch-3', '나는 무작정 미쓰코시 백화점 옥상으로 올라갔소.
...
('ko-jindallae', 0, '진달래꽃 (영변에 약산)', 'jindallae-ch-1', '나 보기가 역겨워
가실 때에는
말없이 고이 보내 드리우리다
...
('ko-jindallae', 1, '산유화 · 엄마야 누나야', 'jindallae-ch-2', '산에는 꽃 피네
꽃이 피네
갈 봄 여름 없이
꽃이 피네
...
('ko-jindallae', 2, '초혼 (招魂)', 'jindallae-ch-3', '산산이 부서진 이름이여!
허공중에 헤어진 이름이여!
불러도 주인 없는 이름이여!
부르다가 내가 죽을 이름이여!
...
('ko-yun-dong-ju', 0, '서시 (序詩)', 'yun-ch-1', '죽는 날까지 하늘을 우러러
한 점 부끄럼이 없기를,
잎새에 이는 바람에도
나는 괴로워했다.
...
('ko-yun-dong-ju', 1, '별 헤는 밤', 'yun-ch-2', '계절이 지나가는 하늘에는
가을로 가득 차 있습니다.
...
('ko-yun-dong-ju', 2, '자화상 (自画像)', 'yun-ch-3', '산모퉁이를 돌아 논가 외딴 우물을 홀로 찾아가선 가만히 들여다봅니다.
...
('ko-unsu-joeun-nal', 0, '제1부 — 뜻밖의 횡재', 'unsu-ch-1', '새침하게 흐린 품이 눈이 올 듯하더니, 눈은 아니 오고 얼다가 만 비가 추적추적 내리는 날이었다.
...
('ko-unsu-joeun-nal', 1, '제2부 — 선술집의 설렁탕', 'unsu-ch-2', '비는 그치지 않고 더욱 굵어졌다. 남대문 정거장까지 데려다 달라는 손님을 태우고 내달린 김첨지는 다리가 부들부들 떨리고 숨이 턱까지 차올랐지만, 손에 쥐어지는 일 원 오십 전이라는 거금에 입이 찢어지는 듯했다.
...
('ko-unsu-joeun-nal', 2, '제3부 — 괴상한 정적', 'unsu-ch-3', '김첨지는 취한 걸음으로 설렁탕을 사 들고 골목길을 돌아 자신의 오막살이집으로 들어섰다.
...
('ko-bom-bom', 0, '키 안 크는 점순이', 'bom-ch-1', '오늘도 또 우리 수탉이 짓밟혔다. 내가 점심을 먹고 나무를 하러 가려는데, 대사리 바깥마당에서 닭의 횃소리가 요란하게 났다. 얼른 달려가 보니, 점순이네 큰 수탉이 우리 작은 수탉을 올라타고 쪼아대는 것이 아닌가.
...
('ko-bom-bom', 1, '장인과의 담판', 'bom-ch-2', '하지만 내가 정말 애가 타는 것은 닭싸움 때문만이 아니었다. 장인님은 점순이의 키가 안 컸다는 핑계로 삼 년 칠 개월이 넘도록 혼인을 시켜주지 않고 나를 머슴처럼 부려먹고 있었다.
